...
public class LessonControllerRestTemplateTest {
    ...
    public void setUp() throws Exception {
        ...
        repository.saveAll(Arrays.asList(
                new Lesson().setTitle(titles[0]).setDeliveredOn(new Date(df.parse("2016-06-11").getTime())),
                new Lesson().setTitle(titles[1]).setDeliveredOn(new Date(df.parse("2016-07-11").getTime())),
                new Lesson().setTitle(titles[2]).setDeliveredOn(new Date(df.parse("2016-08-11").getTime())),
                new Lesson().setTitle(titles[3]).setDeliveredOn(new Date(df.parse("2016-09-11").getTime())),
                new Lesson().setTitle(titles[4]).setDeliveredOn(new Date(df.parse("2016-10-11").getTime()))
        ));
    }
    ...
    public void getAllBetweenDateTest() throws Exception {
        ...
        assertTrue(lessons.stream().anyMatch(e -> dateFrom.compareTo(e.getDeliveredOn()) < 1));
        assertTrue(lessons.stream().anyMatch(e -> dateTo.compareTo(e.getDeliveredOn()) > -1));
    }

    @Test
    public void createNewLessonTest() throws Exception {
        Lesson addedLesson = new Lesson().setTitle("New Title")
                .setDeliveredOn(new Date(df.parse("2016-11-11").getTime()));
